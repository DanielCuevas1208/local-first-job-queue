...
func TestPriorityAgingTiesUseDeterministicOrder(t *testing.T) {
	...
	// Both jobs get the same aging boost, so created_at and then id decide.
	if _, err := s.db.Exec(
		`UPDATE jobs SET created_at = ? WHERE id = ?`,
		time.Now().UTC().Add(-3*time.Second).Format(sqliteTimeFormat), first.ID); err != nil {
		t.Fatalf("backdate first: %v", err)
	}
	if _, err := s.db.Exec(
		`UPDATE jobs SET created_at = ? WHERE id = ?`,
		time.Now().UTC().Add(-3*time.Second).Format(sqliteTimeFormat), second.ID); err != nil {
...
// must move it to the dead-letter state instead of looping forever.
