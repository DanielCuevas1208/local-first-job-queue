...
// DefaultAgingInterval is the recommended priority aging interval. A pending
// job gains one priority point per interval it has waited, which prevents a
// constant high-priority stream from starving lower-priority work. The work
// command uses this value unless the operator overrides it.
...
func (s *SQLiteStore) GetPendingJobs() ([]Job, error) {
	...
	return s.queryJobs("WHERE state = 'pending' ORDER BY "+orderBy, orderArgs...)
}
